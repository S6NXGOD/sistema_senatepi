 ...
 * Ex: `/filiados/<id>/qrcode` ou `/dependentes/<id>/qrcode`.
 */
export function QrCodeDialog({
  ...
  function imprimir() {
    ...
    if (!win) return;
    win.document.write(
      `<html><head><title>${titulo}</title></head><body style="display:flex;align-items:center;justify-content:center;height:100vh;margin:0">
      ...
      <div className="w-full max-w-sm rounded-xl bg-card p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="flex w-full gap-2">
            <a href={data?.imagem} download="qrcode.png" className="flex-1">
              <Button variant="outline" className="w-full" disabled={!data}>
                <Download className="h-4 w-4" /> Baixar
              </Button>
            </a>
            <Button onClick={imprimir} disabled={!data} className="flex-1">
