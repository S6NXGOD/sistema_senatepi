...
/*
  AS REGRAS DE "QUEM ESTÁ DE CADA LADO" SAÍRAM DAQUI — 22/09/2026.

  Foram para `adversario.util.ts`, que não tem decorador nenhum. O motivo está
  escrito lá: importar este arquivo (que é um módulo Nest) de dentro de
  ProcessosModule fecha um ciclo e a aplicação não sobe. A reexportação fica
  para não quebrar quem já importava daqui.
*/
// O próprio módulo também usa as regras: importa e reexporta.
import {
  adversarioDoProcesso,
  ehONossoSindicato,
  nomeCurtoDaParte,
  nossoPolo,
  parteAdversaria,
  partesDaLinha,
} from './adversario.util';
